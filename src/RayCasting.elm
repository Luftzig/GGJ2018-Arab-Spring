...
type alias Ray = Line
type alias Segment = Line

boxCorners : Box -> List Position
boxCorners box =
    let x = box.leftBottom.x
        y = box.leftBottom.y
...
boxWalls box =
    let x = box.leftBottom.x
        ...
        w = box.dimensions.width
        h = box.dimensions.height
    in [ { source = Vector x y, direction = Vector w 0 }
...
raysToBoxCorners source box =
    let walls = boxWalls box
        corners = boxCorners box
        rawRays = List.map (\dst -> lineFromPoints source dst) corners
    in List.map (\ray -> List.foldr cutRay ray walls) rawRays


vectAdd : Vector -> Vector -> Vector
vectAdd (Vector x1 y1) (Vector x2 y2) = (Vector (x1 + x2) (y1 + y2))
vectSub : Vector -> Vector -> Vector
vectSub (Vector x1 y1) (Vector x2 y2) = (Vector (x1 - x2) (y1 - y2))
vectScale : Float -> Vector -> Vector
vectScale s (Vector x y) = Vector (s*x) (s*y)
...
pointsFromLine l =
    ...
    , let p = vectAdd l.source l.direction in { x = vX p, y = vY p})
    ...
    in if (dx1*dy2 ==  dx2*dy1) && (dx2 /= 0)
        then r
            ...
            in if (t1 > 0) && (t1 < 1) && (t2 > 0) && (t2 < 1)
                then { r | direction = vectScale t1 r.direction }
                else r

raysPolygons : List Ray -> List Shape
raysPolygons rays = case rays of
    [] -> []
    [r] -> []
    (r1::r2::rs) ->
        let (s, p1) = pointsFromLine r1
            (_, p2) = pointsFromLine r2
            sx = s.x
            sy = s.y
            rx1 = p1.x
            ry1 = p1.y
            rx2 = p2.x
            ry2 = p2.y
        in polygon [ (sx, sy), (rx1, ry1), (rx2, ry2) ] :: raysPolygons (r2::rs)

rayAngle : Ray -> Float
rayAngle ray = (\(_,phi) -> phi) <| toPolar (let d = ray.direction in (vX d, vY d))




source : Position
source = Position 150 110
...
rays0 : List Ray
rays0 = raysToBoxCorners source box
